...
<h3> replacement output </h3>
	...
	$inittemp = $_GET["init"];
	$responsetemp = $_GET["response"];
	$init = explode(" ",$inittemp);
	$response = explode(" ",$responsetemp);
	$initnum = 0;
	$responsenum = 0;
	foreach($init as $x)
	{
	...
	foreach($response as $x)
	{
		...
		if(floatval($x) != 0)
		{
			$responsenum = floatval($x);
			break;
		}

	}
	if($initnum > $responsenum)
	{
		echo "the gpa in the statement \"".$inittemp."\" is greater than the statement \"" . $responsetemp. ".\""; 
	}
	else if($initnum < $responsenum)
	{
		echo "the gpa in the statement \"".$inittemp."\" is lesser than the statement \"" . $responsetemp. ".\""; 
	}
	else
	{
		echo "the gpa in the statement \"".$inittemp."\" is equal to the statement \"" . $responsetemp. ".\""; 
	}
	?>
